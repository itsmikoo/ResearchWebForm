...
export default function App() {
  ...
  useEffect(() => {
    if (appState === 'done') {
      // Jangan lupa kirimkan juga data grup ke database!
      console.log("Submitting to DB:", { 
        participant_name: participantName, 
        group_type: groupType, // Kolom penting untuk analisis ANOVA-mu nanti!
        responses 
      });
      // submitSurveyData({ participant_name: participantName, group_type: groupType, responses });
    }
  ...
  return (
    <div className="min-h-screen bg-[#fdf8f0] flex items-center justify-center md:p-6 font-sans text-slate-800">
      <div className="bg-white w-full max-w-4xl min-h-screen md:min-h-[600px] md:rounded-2xl shadow-none md:shadow-xl p-6 sm:p-8 md:p-12 flex flex-col relative overflow-hidden">
        ...
        {appState === 'welcome' && (
          <WelcomeScreen onNext={() => setAppState('tutorial')} setName={setParticipantName} />
        )}

        {appState === 'tutorial' && (
          // Lempar data grup ke tutorial agar instruksinya menyesuaikan
          <TutorialScreen group={groupType} onFinish={() => setAppState('ready')} />
        )}
