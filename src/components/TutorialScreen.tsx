...
export default function TutorialScreen({ group, onFinish }: TutorialProps) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto flex flex-col flex-1 relative">
      ...
      <div
        className={`absolute z-[60] w-full sm:w-[340px] bg-white rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.25)] p-5 border border-amber-100 flex flex-col transition-all duration-500 ease-in-out ${panelPosition}`}
      >
        ...
        <div className="text-slate-600 text-sm leading-relaxed min-h-[5rem] mb-4">
          {step === 1 &&
            "⏱️ Watch your question progress and the timer. Time runs automatically, but don't panic—go at your own comfortable pace."}

          {step === 2 &&
            "🔎 This is the email content area. Read the context and details carefully to understand the message."}

          {isAIStep && (
            <span>
              ✨ <strong>AI Assistant:</strong> Words highlighted in{" "}
              <span className="text-green-600 font-semibold">Green</span>{" "}
              indicate safe patterns, while{" "}
              <span className="text-red-600 font-semibold">Red</span> indicates
              phishing.
              <br />
              <br />
              <em>
                Tip: Darker colors mean stronger AI confidence. Try hovering
          ...
          {isButtonStep &&
            "👆 Once you've made up your mind, click one of these buttons to submit your decision."}

          {/* Instruksi Tambahan Khusus Fase Evaluasi */}
          {isEvalStep &&
            "📊 After deciding, the timer will pause. You must rate how confident you are with your answer before moving to the next question."}
        </div>
      ...
      <div
        className={`transition-all duration-300 ${step === 1 ? "relative z-50 bg-white ring-4 ring-amber-500 shadow-2xl rounded-xl p-4 -mx-4" : "relative z-30 p-4 -mx-4 mb-2"}`}
      >
        <div className="flex gap-1 sm:gap-2 mb-6">
          {[...Array(10)].map((_, i) => (
            <div
              key={i}
              className={`h-1.5 flex-1 rounded-full ${i === 0 ? "bg-amber-400" : "bg-gray-200"}`}
            ></div>
          ))}
        </div>
      ...
      <div
        className={`transition-all duration-300 mb-8 sm:mb-10 rounded-xl ${isEmailHighlighted ? "relative z-50 ring-4 ring-amber-500 shadow-2xl" : "relative z-30 opacity-40"}`}
      >
        {group === "control" && (
      ...
      {!isEvalStep ? (
        ...
        <div
          className={`transition-all duration-300 flex flex-col sm:flex-row gap-3 sm:gap-4 w-full mt-auto ${isButtonStep ? "relative z-50 bg-white ring-4 ring-amber-500 shadow-2xl rounded-xl p-4 -mx-4" : "relative z-30 p-4 -mx-4 opacity-40"}`}
        >
        ...
        <div
          className={`transition-all duration-300 w-full mt-auto ${isEvalStep ? "relative z-50 bg-slate-50 ring-4 ring-amber-500 shadow-2xl rounded-2xl p-5 md:p-6 -mx-4" : "hidden"}`}
        >
          <div className="text-sm font-semibold text-slate-500 border-b border-slate-200 pb-2 mb-4">
            Your Answer: <span className="text-red-600">🔴 Phishing</span>{" "}
