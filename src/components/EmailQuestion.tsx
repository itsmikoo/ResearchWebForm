...
export default function EmailQuestion({ pageNumber, group, onAnswer }: EmailProps) {
  ...
  const displayTime = (isEvaluationPhase ? frozenTime / 1000 : timeMs / 1000).toFixed(1);
  ...
  return (
    <div className="w-full max-w-3xl mx-auto flex flex-col flex-1">
      ...
      <div className="mb-8 sm:mb-10 overflow-y-auto max-h-[50vh] sm:max-h-none rounded-xl">
        {group === 'control' && (
          <ControlEmailViewer text={currentEmail.text} />
        )}
      ...
      {!isEvaluationPhase ? (
        ...
        <div className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 md:p-6 mt-auto shadow-inner flex flex-col gap-5 animate-fadeIn">
          <div className="text-sm font-semibold text-slate-500 border-b border-slate-200 pb-2">
            Your Answer: <span className={mainDecision ? "text-red-600" : "text-green-600"}>{mainDecision ? "🔴 Phishing" : "🟢 Safe"}</span>
          </div>
            ...
            <div className="flex justify-between items-center gap-1 sm:gap-2 mt-1">
              ...
              {[1, 2, 3, 4, 5].map((num) => (
                <button
                  key={num}
                  type="button"
                  onClick={() => { setConfidence(num); setError(""); }}
          ...
          <div className="border-t border-slate-200 pt-3 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="h-5 text-center sm:text-left">
              {error && <p className="text-red-500 text-xs sm:text-sm font-bold">⚠️ {error}</p>}
            </div>
            <button
              type="button"
            ...
            >
              {pageNumber === totalQuestions ? "Go to Final Evaluation 📋" : "Next Question →"}
            </button>
          </div>
        </div>
      )}
